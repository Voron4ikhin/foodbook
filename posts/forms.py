...
class PostModelForm(forms.ModelForm):
    ...
    components = forms.CharField(label='Ингредиенты', widget=forms.Textarea(attrs={'rows': 3, 'placeholder': 'Молоко - 500мл,\nМука - 200гр,'}))
    image = forms.ImageField(label='Фото', required=False)

    class Meta:
        model = Post
        fields = ('name', 'components', 'content', 'image')


class CommentModelForm(forms.ModelForm):
    body = forms.CharField(label='', widget=forms.TextInput(attrs={'placeholder': 'Написать комментарий...'}))
